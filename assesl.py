...
class an_ass:
    ...
    def __init__(self,file,ud):
        ...
        nnc=[]
        for line in nc:
            if line.count("-")==1 and (line.count(",-")==1 or line.count(" -")==1):
                nnc.append(line.replace("-",""))
            else:
                nnc.append(line)
        content="\n".join(nnc)

        if ud == 'u':
            self.style='Style: '+ud+',NotoSansSC-Medium,25,&H00FFFFFF,&H00FFFFFF,&H00000000,&H00000000,0,0,0,0,100,100,0,0,1,1,0,2,10,10,15,0\n'
            ...
            self.style='Style: '+ud+',NotoSansSC-Medium,13,&H00FFFFFF,&H00FFFFFF,&H00000000,&H00000000,0,0,0,0,100,100,0,0,1,1,0,2,10,10,5,0\n'
            for p in content.split('\n'):
                if p.split('0,0,0,,')[-1] and p.split('0,0,0,,')[-1].strip()!='-' and p.startswith('Dialogue'):
